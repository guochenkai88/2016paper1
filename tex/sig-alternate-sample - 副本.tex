...
///////////////// implementing corresponding an phishing attack app aiming at "webchat" app and a spoofing app on "ibank" app.  Then, a prevalent recent attack in PC called "ransomware" also can be engaged under the callback threats. Again, we develop corresponding malware of "ransomware" to indicate the feasibility of such attack. In order to check the influence of callback threats, we have completed an assessment implementation to automatically handle the detection. As our detection on a large scale of existing apps on Android market, the callback threats widely exist. Of all {\color{red}one million} apps downloaded from Google Play and other Android app markets, {\color{red}30\% } have this sort of callback structure vulnerability. which has constituted serious impact to users.

We also discuss the defence strategy aiming to the callback threats.
...
Android platform is a kind of open source operating system established by Linux core. Developers could design and develop their own app product on Android in a flexible way, causing Android freely provides a variety of open source libraries and frameworks for developers. Of which, four types of components plays the key role in Android app's construction, namely \textit{Activity}, \textit{Service}, \textit{Broadcast Receiver} and \textit{Content Provider}. \textit{Activity} is designed as a graphical interface for interacting with users. Nevertheless, \textit{Service} is mainly used for handling complex computing job in background without a graphical user interface. \textit{Broadcast Receiver} provides a global listener that could trigger specific code logic when receive relative message. \textit{Content Provider} providers a universal interface for data storage and management, and for sharing data between different components. 

/////////////// The permissions mechanism as another protection mechanism is born along with the Android sandbox.Apps would be asked for requesting specific permissions when any of their functionalities involves any sensitive resources within the device environment. However, not all the permissions are able to attract user's concern.In fact, only very few permissions directly related to the private information such as location or SMS can arouse attention from ordinary users.According to this consideration, we ranked all of current permissions offered by Android 5 through a statistic algorithm on 1260 malicious apps[5] and 12000 normal apps on Google Play, and chose the permissions with {\color{red}5 highest risk} as alarm-permissions,{\color{red}{ showed in Figure1}}. As expected, {\color{red}{the selected alarm-permissions}} are all user-concerned in intuition.

\subsubsection{Callback Categories}
As a side-effect of user-driven mechanism, Android provides numerous of callback interfaces to be implemented by generic developers. Different from conventional PC program launched by the \textit{main} function, callback function could only be executed when a specific event happens. The trigger-events originated by different sources are equipped with different trigger features. For instance, a \textit{low-battery} event would be triggered only when the rest quantity of battery becomes as low as a pre-defined level, which is irrelevant to user's interaction. We categorize the callback functions as three groups in term of their trigger events, namely \textit{life-cycle} callbacks, \textit{user-driven} callbacks, and \textit{system-driven} callbacks.

\textbf{Life-cycle callbacks} Android apps would spontaneously arouse corresponding callback function according to the life-cycle step it resides. For instance, an Activity executes its \textit{onCreate} function only when it is initially loaded, and executes the \textit{onPause} callback after it loses the screen focus. Life-cycle callbacks are somehow executed along with user's interactions and impacted by users. However, it performs fairly transparent to users, as a part of internal mechanism of Android system. Due to constraint of sandbox mechanism, the life-cycle phrases (Create,Pause,Destroy, etc.) of an given activity are entirely isolated from components belong to other apps. That is to say, any app component is unable to capture the life-cycle statuses from other apps. This sets up a huge obstacle for adversary app that wants to conduct attacks over target benign app.

\textbf{User-driven callbacks} User-driven callbacks interact with users in a direct way. User regularly are guided to perform diverse actions on screen, such as tapping, dropping up and down, click, double-click, etc., to operate a new app. Right after capturing an action from users, Android system would react following the  corresponding callback function that reflects the functionality and logic designed by developer. Generally, 

\textbf{System-driven callbacks} System-driven callbacks are normally triggered by a given system event which could be captured when a given system status changes. Not being the dependency of any apps, system status could be obtained by any app in theory so long as one is granted proper permissions. Generally, the permission granting could hardly attract user's attention for system event such as battery status is insensitive to user. To the end, system event provides an indirect entry for adversary app getting parts of the running process of other apps that implement system-driven callbacks interfaces.

\subsubsection{Callback and Components}
\textbf{Callback in Activity} 
Most of callbacks in Activity are user-driven and life-cycle, causing activity is mainly used for providing a visible graphic interfaces so as to interact with users. As aforementioned, upon the protection of sandbox, these callback statuses are hardly obtained by other apps only if the event triggered by user is irrelevant to screen focus. For instance, a shake action from users would lead to \textit{sensor} related events, which have no especial association with the screen. Besides, still some system-driven callbacks are implemented in activity for feeding the requirements of functionalities. 

\textbf{Callback in Service} 
Handling program logic and computing tasks in background, Service has no direct interaction with users, which is typically different from Activity. By this means, callbacks from service only come from life-cycle and system statuses. Yet, user action without screen focus could be an exception, which we will discuss later.
...
It is also possible to trigger a callback through an user-driven event in service, which is irrelevant to visible screen (e.g., shaking the phone to trigger SensorChanged event). Without specified receiver, this kind of events could be easily obtained by other apps either.   
...
Table1 lists typical broadcast event examples.
\begin{table}[htbp]
\centering
 \caption{\label{tab:test}System event callback}
 \begin{tabular}{lcll}
  \toprule
  Event & Callback & Permission & Usage\\
  \midrule
  Intent.ACTION\_BATTERY\_LOW & onReceive & N/A & The battery level has fallen below a threshold\\
  Intent.ACTION\_BOOT\_COMPLETED & onReceive & N/A & Android is up and running\\
  Intent.ACTION\_DEVICE\_STORAGE\_LOW & onReceive & N/A & Storage space on the device is getting limited\\
  \bottomrule
 \end{tabular}
\end{table}
...
%Each Activity has a regular life-cycle sequence specified by a series of typical callback functions(onCreate, onPause, onDestrory, etc.)that offers  developers interfaces to implement corresponding functionality. 
...
\caption{\label{} Motivating example about Wechat app. (a) presents the "Shake" preparation page; (b) shows a real result of the "Shake"; (c) presents the information of such real page; (d) shows a fake "result" page linked from the "Shake"; (e) presents the fake information of the fake result.}
\end{figure*}

We take the popular app Wechat as a motivating example. Wechat is known as a popular IM(Instant Messenger) tool for users sharing information and connecting with each other. Of diverse functionalities it provides, "YAO-YI-YAO" is rather welcomed, which enables users to randomly obtain an online chatting partner so long as slightly shaking the device several times, as shown in {\color{red}Figure 1}.

With harmony in surface, there exists a potential crisis that the user shaking action could be easily perceived by any other app through the {\color{red}SENSOR\_SERVICE}, a system service for managing inner device sensor. That is to say, only needing to instantiate the {\color{red}sensormanager} object and arrange the shaking margin , an adversary app could clearly receive the same action event signal as Wechat. Further more, associating with the running status of Wechat through {\color{red}AMS(Activity Manager Service)}, adversary app could naturally make a judgement about the accurate time when the Wechat responding page would be popped up after user shaking. Although there is an exception that user shakes the device without "YAO-YI-YAO" page residing top location of task stack.  Yet, user shakes device in such wide margin normally with intensive intention, so this exception case is scarcely possible in practice.

It seems that this kind of event leakage that we call it callback time leakage is easily ignored by developers and app markets. However, a wily attacker could devise diverse attacks utilizing the callback time leakage. For instants, attackers could deliberately construct a similar result page that links to a malicious third-party chatting tool, or even mimic a fake chatting page to pretend to chat with user for malicious purpose.

///////////We see the callback time leakage severely because it offers a big change for attackers. 


\subsection{Callback State Model} 

\# We utilize the task state transition model in[100] to illustrate the task transition process in callback attacks. 
It is worth noting that there is only one back stack in our model, causing the task hijacking[100] is not taken into consideration in this paper.

In order to clearly illustrate the callback threats, we view the threats process as a kind of callback state model. Each callback within a installed app can be described as (E,S,F,C), where "E" denotes the event that invokes executing of the callback function; "S" denotes current state of app, which can be observed by other apps; "F" denotes the final sink contained by the callback function; "C" denotes the proper control flow conditional values towards the "F".

1. \textbf{Event}(e belongs to E).  The event here refers to those can be observed by public. For example, normally user-click event does not belong to the set, causing it generally only can be preserved by local app components; other app is forbidden to obtain this event due to the isolation of sandbox.

2. \textbf{State}(s belongs to S). The State refers to current program state of an app, through which the running location can be  inferred. Similar to the Event, the State here also needs to be exposed to other apps. 

3. \textbf{Display Sink}(f belongs to F). Display Sink is based on the conception of leakage detection, which refers to a set of label functions that can expose data or information to outside. However, different from traditional conception, Display Sink here specifically refers to state output that could be perceived by users rather than information leakage, including UI page, dialog or notification, etc.

4. \textbf{Control Flow}(C). Control Flow is a collection of conditional values over the path from a typical State to a Final Sink. In theory, as for a callback threat vulnerable app, the "C" needs to guarantee an observable and feasible path that can reach the Final Sink. However, the "C" that strictly conforms above definition rarely emerges in real apps, so that we adopt a more flexible mechanism in practice which will be introduced in next chapter.  

\subsection{Callback Vulnerability Example} 

We takes the memory Event as an example to depict the callback threats in detail. As the code snippet shown in {\color{red}Figure 2}, it is a typical OnReceiver callback function would be invoked right after a memory-changing event happens. 
...
\centering
 ...
 \begin{tabularx}{\linewidth}{XXXX}
  ...
  \midrule
 1.Activity running state(Lauching page) & 1.startActivity & 1.Broadcast from system: \{Intent.ACTION\_BATTERY\_LOW\} & 1.branch or cycle condition value \\
  2. Service running state &2. Toast message & 2.Implicit Intent\& pendingIntent & 2.inter-procedure condition value\\
...
Before {\color{red}Android 5.0} emerging, the execution state of component like Activity and Service can be easily observed by the {\color{red}getRunningTasks method in ActivityManager object}. A more detailed observation needs to request the "GET\_TASK" permission, which could somehow attract the attention of most of normal users. Unfortunately, Android starts to constraint the usage of this permission from recent {\color{red}Android 5.0} for the security consideration. The "REAL\_GET\_TASK" permission is only available to system apps; while normal third-party app is forbidden to request.  {\color{red}An substitute solution is to request new "PACKAGE\_USAGE\_STATS" permission, and to disable warning that permission is system level and will not be granted to third-party apps. Again, this approach needs extra permission request. }

Although it is difficult to obtain the accurate running task above the version of 5.0, the running application package name is still easily obtained by getAppRunningProcess in ActivityManager. The good news is that the usage of "getAppRunningProcess" and "getRunningServices" are still available in the newest version and need not request any extra permission. Thus, we can judge if a typical app is running through the "getAppRunningProcess"[101]. Through a continuing such observation to a target app, we can determine if its launching activity is running on foreground. More fine-grained, the "getRunningServices" can be used to distinguish which service is running in background.

{\color {blue} There is a kind of AccessibilityService that can capture other apps' states like "click, window change, input, etc.". However, it needs to let user grant the "service" in setting. So we could design a permission-escalation spoofing to enable the  AccessibilityService.}

----------------------------------------------

As to the Final Sink, only those output vectors which can be leveraged by attackers are considered. Most of which are related to UI activity, causing the UI is designed as the main channel to interact with users. Some of the UI vectors are mentioned in recent research[102](e.g., startActivity API, Toast message), yet we also collect some other UI related vectors, e.g., AlertDialog.Builder API, NotificationManager API. Generally, both of Dialog frame and Notification need response from users(e.g., click) and then engage corresponding reaction functionalities. This process offers chance for attackers to interfere the users behaviours, which is insecure.
...
\paragraph{Program Control Flow}
...
5 CS = "jiao" ( cv(a) \& s(a) ) = ("jiao"(c)) \& ( "jiao" s(a))
...
vectors
1. battery  2. sensorManager 3. timer 4. implicate Intent 5. memory 6....

features 1. toast 2. dialogue 3. Activity vision 4. camera 5. voice 6. notify 7....
